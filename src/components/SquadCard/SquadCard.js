...
const SquadCard = (props) => {
  ...
  return (
    user && (
      <Card className="card squad-card mb-3" key={id}>
        <Card.Body className="card-body">
          ...
          <div className="squad-card-buttons">
            ...
            {showingPoke &&
              !squad.pokemon.some((poke) => poke._id === showingPoke._id) && (
                <button
                  className="add-to-squad-btn"
                  onClick={() => onAdd(id, showingPoke._id, user)}
                >
                  Add {showingPoke.name} to this Squad
                </button>
              )}
            <button
              className="visit-squad-btn"
              onClick={() => history.push(`/squads/${props.id}`)}
            >
              Visit Squad
            </button>
          </div>
