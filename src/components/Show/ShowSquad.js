...
const ShowSquad = (props) => {
  ...
  useEffect(() => {
    !squad &&
      showOneSquad(match.params.id, user)
  ...
  return (
    <div>
      <Button
        className="squad-show-delete-button"
        variant="danger"
        onClick={() => deleteThisSquad(squad._id, user)}
      >
        Delete this Squad
      </Button>
      ...
      {squad && squad.pokemon.length === 0 && (
        <h1 className="empty-squad">
          You have no pokemon in this squad! To to the store to add some.
        </h1>
      )}
    </div>
  );
